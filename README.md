...
- 📅 **日历视图** - 月历热力图展示打卡记录，支持月份切换
- 📁 **多项目管理** - 支持创建和管理多个目标项目
- 🔥 **连续打卡统计** - 显示当前连续打卡天数和最长记录
- 🎨 **六种主题颜色** - emerald、cyan、amber、violet、rose、orange
- 📝 **备注功能** - 右键点击已完成的格子添加备注
- 🌙 **深色模式** - 支持浅色/深色主题切换
- 💾 **数据导出导入** - JSON 格式备份和恢复数据
- 🎉 **里程碑庆祝** - 达成 25%/50%/75%/100% 时触发庆祝动画
- 📱 **响应式设计** - 完美适配桌面端和移动端
...
- 🎉 **里程碑庆祝动画** - 进度达成时触发
  - 25%、50%、75%、100% 四个里程碑
  - 彩纸飘落庆祝动画（使用 canvas-confetti）
  - 祝贺弹窗显示进度和鼓励文字

**技术改进：**

- 扩展 `projectStore` 添加日历、统计、导入导出方法
- 新增 `calendar` 组件目录，包含日历相关组件
- 新增类型定义：`CalendarDay`、`CalendarMonth`、`StreakInfo`、`ExportData` 等
- 新增依赖：`canvas-confetti` 用于庆祝动画
